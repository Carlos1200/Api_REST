...
switch ($method) {
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare("INSERT INTO profesores (nombre, apellido, edad, asignatura) VALUES (?, ?, ?, ?)");
        $stmt->execute([$data['nombre'], $data['apellido'], $data['edad'], $data['asignatura']]);
    ...
    case 'PUT':
         ...
         if ($data['edad']==-987) {
           ...
           $stmt = $pdo->prepare("UPDATE profesores SET nombre = ?, apellido = ?, edad = ?, asignatura= ? WHERE id = ?");
           $stmt->execute([$data['nombre'], $data['apellido'], $data['edad'], $data['asignatura'], $data['id']]);
           $profesor = [
            'id' => $data['id'],
            'nombre' => $data['nombre'],
            'apellido' => $data['apellido'],
    ...
    case 'DELETE':
       ...
       if (isset($_GET['id'])) {
          ...
          echo json_encode(['error' => 'No se proporcionado un ID del alumno para actualizar']);
        }
        break;
    default:
