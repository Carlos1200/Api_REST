...
import com.example.apirest.profesor.TeacherActivity
...
class AlumnoActivity : AppCompatActivity() {
                    ...
                    if (alumnos != null) {
                        ...
                        adapter.setOnItemClickListener(object : AlumnoAdapter.OnItemClickListener {
                            override fun onItemClick(alumno: Alumno) {
                                ...
                                AlertDialog.Builder(this@AlumnoActivity)
    ...
    private fun eliminarAlumno(alumno: Alumno, api: AlumnoApi) {
        ...
        Log.e("API", "id : $alumno")
        val llamada = api.eliminarAlumno( alumnoTMP)
        llamada.enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@AlumnoActivity, "Alumno eliminado", Toast.LENGTH_SHORT).show()
                    cargarDatos(api)
                } else {
                    val error = response.errorBody()?.string()
                    Log.e("API", "Error al eliminar alumno : $error")
                    Toast.makeText(this@AlumnoActivity, "Error al eliminar alumno 1", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<Void>, t: Throwable) {
                Log.e("API", "Error al eliminar alumno : $t")
                Toast.makeText(this@AlumnoActivity, "Error al eliminar alumno 2", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_profesores -> {
                val intent = Intent(this@AlumnoActivity, TeacherActivity::class.java)
    ...
    fun logout(item: MenuItem) {
        FirebaseAuth.getInstance().signOut().also {
            val intent = Intent(this@AlumnoActivity, MainActivity::class.java)
            startActivity(intent)
            finish()
        }
    }
